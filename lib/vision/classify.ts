...
TRACE THE EDGES. Landscape beds are curved, and a polygon of a few vertices cuts straight chords across a curve — the outline then covers lawn on one side and misses bed on the other, which is exactly what the customer sees when they swap the material. Use as many vertices as the edge needs: 6 is fine for a rectangular driveway, and a curved bed edge usually wants 20-40. Put them where the edge changes direction, not at even intervals. If any part of your outline is a straight line where the real edge curves, that part needs more vertices. Follow the real boundary — the mulch/lawn line, the edge of the concrete — rather than drawing a convex hull around the area; regions are often long, thin, or crescent-shaped, and that is fine.

STOP INSIDE THE EDGING. Most beds are bounded by a hard border: a row of river-rock cobbles, steel or aluminium edging, brick, a paver soldier course, a concrete curb. **That border is not part of the bed.** Trace the INNER edge of it — where the mulch actually stops — so the outline sits inside the border with the border itself outside. This matters more than it sounds: the region is what gets re-surfaced when the homeowner swaps mulch for stone, and an outline drawn across a cobble border paints gravel over the border they already have.
