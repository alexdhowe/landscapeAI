...
| The database, against real Neon | ✅ **done for real** — the "Database setup" Action applied all 12 migrations to a free Neon project in ~4s through the *pooled* endpoint, seeded the price book and created the first contractor login, in a 25-second run. The connection string went in exactly as Neon's console hands it out; without the stripping in the row above, that step would have hung until the 10-minute cap with an empty log |
